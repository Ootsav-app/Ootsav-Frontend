...
const EventTimeCard: React.FC<EventTimeCardProps> = ({
  ...
  const CountdownItem = ({ value, label }: { value: number; label: string }) => (
    <div className="flex flex-col items-center mx-1">
      ...
      <span className="text-xs font-semibold text-white/90 uppercase mt-1">{label}</span>
    </div>
  );
    ...
    <div className="m-4 bg-white rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.08)] overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 flex justify-between items-end">
          <div className="flex flex-col">
            ...
            {timeLeft ? (
              ...
              <span className="text-white text-2xl font-bold">Event Started!</span>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="p-4 pb-6">
        <div className="flex justify-between items-start mb-2">
          <h2 className="text-2xl font-bold text-gray-900 truncate pr-2">{names}</h2>
        </div>
        ...
        <div className="flex items-center space-x-4 overflow-x-auto no-scrollbar">
          ...
          <div className="flex items-center text-gray-600 whitespace-nowrap cursor-pointer hover:text-red-500 transition">
            <MapPin className="w-5 h-5 mr-1 text-gray-500" />
            <span className="font-semibold text-lg truncate max-w-[150px]">{location}</span>
          </div>
        </div>
      </div>
    </div>
